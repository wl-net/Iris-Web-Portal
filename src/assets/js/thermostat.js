...
function onMthermostat(evt) {
	if(evt.data.search("6606672e-57f8-47d1-8002-5fe59d34c1d8") != -1){
		listthermostat(evt.data)			
	}
}
function listthermostat(devices){
		...
		for (var i = 0; i < Devices.length; i++) {
			var device = Devices[i].split(",");
				for (var x = 0; x < device.length; x++) {
					...
					+ ' <div class="tooltip">' + devBatteryNum + '<span class="tooltiptext"><div id="BATT' + devID + '">' + devBattery + '</div></span> </div> '
					+ '<div class="tooltip"> <i onclick="resetAC(' + devIDlist.length + ')" class="fa fa-history"></i><span class="tooltiptext"> Reset AC filter timer</span></div> <div class="tooltip"><i onclick="window.location=' + devSettingsLink + ' class="fa fa-cogs"></i> <span class="tooltiptext">Settings</span> </div><br>'
					+ devFav
					+ devPresence 
